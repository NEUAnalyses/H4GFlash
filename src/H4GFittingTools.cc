...
#include <string>
#include "TObjArray.h"
#include "flashgg/H4GFlash/interface/H4GFittingTools.h"
    ...
    for ( unsigned int ff = 0; ff < functionsToFit.size(); ff++) {
 	std::cout << "PRINTING FUNCTION: " << functionsToFit[ff] << std::endl;       
        ...
        for (unsigned int comp = 0; comp < components.size(); comp++){
            ...
            w->pdf( modelName )->plotOn(frame, Components( components[comp] ), LineColor( colors[funcCounter] ), LineStyle( styles[comp] ), Name(components[comp]));
            leg->AddEntry(frame->findObject( components[comp]), components[comp], "l");
        }
	funcCounter++;
...
//std::vector<H4GFittingTools::FitRes> H4GFittingTools::FitFunctions(RooWorkspace* w, std::vector<std::string> functionsToFit, RooDataSet* data, std::string sVar, TH1F* dataHist)
std::vector<H4GFittingTools::FitRes> H4GFittingTools::FitFunctions(RooWorkspace* w, std::vector<std::string> functionsToFit, RooDataSet* data, int isSignal)
{
