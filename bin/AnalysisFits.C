...
//      RooDataSet* dataSig = new RooDataSet( "dataSig", "dataSig", treeSig, *TreeVars, cut.c_str(), "evWeight" );
      ...
      std::vector<H4GFittingTools::FitRes> fitresults = H4GFittingTools::FitFunctions(w, signal_functionsToFit, dataSig, 1);
      ...
      std::vector<H4GFittingTools::FitRes> bkgresults = H4GFittingTools::FitFunctions(w, functionsToFit, data);
      for( unsigned int ftp = 0; ftp < functionsToPlot.size(); ftp++) {
	std::cout << functionsToPlot[ftp] << std::endl;
	}
      for( unsigned int i = 0; i < vars.size(); i++){
          std::string sVar = ( (TObjString *) ( (TObjArray *) (TString(vars[i]).Tokenize("[")) )->At(0) )->String().Data();
          H4GFittingTools::PlotCurves(plotTitle, w, functionsToPlot, legends, bkgresults, data, sVar, nbins[i], plotsDir+"/bkg"+sVar+ TString(plotTitle).ReplaceAll(" ", "_").Data(), 0, 1);
      }
