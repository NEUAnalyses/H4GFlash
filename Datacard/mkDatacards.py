...
if opt.samplesFile == None :
   print " Please provide the samples file ... sereiously no samples?? " 
...
for cutName in cuts :
  ...
  if not os.path.isdir (outputDirDatacard + "/" + cutName) :
    os.mkdir(outputDirDatacard + "/" + cutName)
  
  tagNameToAppearInDatacard = cutName
  
  
  cardPath = outputDirDatacard + "/" + cutName + "/datacard.txt"
